...
retries=10; echo 'Waiting for SQL Server to start...'; until (echo quit | /opt/mssql-tools/bin/sqlcmd -S 127.0.0.1 -l 1 -U sa -P JoomlaFramework123 &> /dev/null) do if [[ "$retries" -le 0 ]]; then echo 'SQL Server did not start';exit 1; fi; retries=$((retries - 1)); sleep 2s; done; echo 'SQL Server started'
